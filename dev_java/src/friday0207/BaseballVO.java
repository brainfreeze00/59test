...
//	}
	...
	//setter메소드의 파라미터로 사용자가 입력한 값 혹은 선택한 정보 또는 결정된 값들이 넘어온다.
	//그런데 이값들은 지변에 담기게 되어서 메소드 영역밖에서는 사용이 불가하다
	//이때 지역변수에 담긴 정보를 전역변수에 다시 초기화 해주면 클래스 전역에서 사용가능함.
	//또 내안이 아니더라도 다른 클래스에서 인스턴스화하면 전역변수는 그대로 누릴수있다.
	//초기화 공부는 책으로는 한계가 있다.
