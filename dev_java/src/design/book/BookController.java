...
public class BookController {
	...
	public BookVO send(BookVO pbVO) { //접근제어 public 타입은 BookVO인 메소드 send에 pbVO를 파라미터
		...
		//DELETE FROM book2020 WHERE b_no=1
			int result = 0;
			result = bDao.bookDelete(pbVO); // create method 키로 Dao에 자동으로 메소드 생성
			...
			//VALUES(?,?,?,?,?)
			int result = 0;
			result = bDao.bookInsert(pbVO);// create method 키로 Dao에 자동으로 메소드 생성
			rbVO.setReuslt(result);
		}
			...
			//WHERE b_no =2
			int result = 0;
			result = bDao.bookUpdate(pbVO);// create method 키로 Dao에 자동으로 메소드 생성
			rbVO.setReuslt(result);
		}
		else if(_SEL.equals(command)) {
			//SELECT b_no, b_name, b_author, b_publish FROM book2020
			//WHERE b_no=3
			rbVO = bDao.bookDetail(pbVO);// create method 키로 Dao에 자동으로 메소드 생성
		}
		return rbVO;
	}////send
	//전체 조회
	public List<BookVO> sendALL(BookVO pbVO){
		...
		if(_ALL.equals(command)) {
			bList = bDao.bookList(pbVO);// create method 키로 Dao에 자동으로 메소드 생성
